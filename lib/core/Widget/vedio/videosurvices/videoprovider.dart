...
import 'package:interactive_e_book_system/core/modules/Videoresponse.dart';
...
class VideoProvider extends ChangeNotifier {
  ...
  Future<void>fetchvideodata()async {
    isLoading = true;
    notifyListeners();
    Videoresponse videoresponse = (await videoviewModel.loadvediodetails(selectedTitle!)) as Videoresponse;
    isLoading = false;
    notifyListeners();
    url=videoresponse.url?? 'https://flutter.github.io/assets-for-api-docs/assets/videos/bee.mp4';
    notifyListeners();
    keywords=videoresponse.keywords??[];
    audioList=(videoresponse.audioList??[]).cast<AudioList>();
    vttList=(videoresponse.vttList??[]).cast<VttList>();
    availableLanguages=(videoresponse.availableLanguages??[]).cast<AvailableLanguages>();
    // data=Data();
    notifyListeners();
